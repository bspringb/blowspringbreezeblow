...
**objects 폼**:
- **attributes 행**: 키 입력창에 커서를 두면, **같은 타입의 기존 객체들이 실제로 썼던 키**가 자동완성으로 뜹니다(`/api/meta`가 타입별로 모아서 내려줌). 값은 숫자처럼 보이면 자동으로 숫자로, 아니면 문자열로 저장됩니다.
...
- 특정 타입을 다른 **실제 타입** 아래로 묶고 싶을 때(예: "감독"은 "영화" 아래에만 — "영화" 자신도 실제 객체가 있는 타입), `objectNav.ts`의 `TYPE_CHILDREN` 레지스트리에 한 줄 추가하면 됩니다:
  ```ts
...
- 반대로 부모 라벨 자체는 **실제 타입이 아니고 화면에만 존재하는 묶음**이고 싶을 때(예: "음악"이라는 객체는 없고, 그 아래 "아티스트"/"앨범"/"곡"만 실제 타입인 경우), `VIRTUAL_GROUPS` 레지스트리를 씁니다:
  ```ts
  export const VIRTUAL_GROUPS: Record<string, string[]> = {
    음악: ['아티스트', '앨범', '곡'],
  };
  ```
  소속 타입 중 실제 객체가 하나도 없으면 이 그룹 자체가 메뉴에서 안 보입니다. **주의**: `VIRTUAL_GROUPS`의 라벨 이름과 같은 문자열을 실제 `type:`으로 쓰면 메뉴에 같은 이름이 두 번 뜹니다 — 라벨은 실존하지 않는 이름으로 고르세요.
